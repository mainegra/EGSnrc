...
/* Output option for total or primary kerma */
enum OutputKind {total, primary};
...
public:
    ...
    ~EGS_KermaApplication() {
        if (ngeom > 0) {
            ...
            if (kerma_p) {
                for (int j=0; j<ngeom; j++) if (kerma_p[j]) {
                        delete kerma_p[j];
                }
                if (kerma_p) {
                    delete [] kerma_p;
    ...
    int ausgab(int iarg) {
        ...
        if (iarg == BeforeTransport && !iq && ir >= 0) {
            ...
            /* Kill photons in exclusion regions */
    ...
    int outputData() {
        ...
        for (int j=0; j<ngeom; j++) {
            ...
            if (kerma_p[j] && !kerma_p[j]->storeState(*data_out)) {
        ...
        if (flug) {
            for (int j=0; j<ngeom; j++) {
                if (!flug[j]->storeState(*data_out)) {
                    return 108+4*(ngeom+j);
                }
                if (!flugT[j]->storeState(*data_out)) {
                    return 108+8*(ngeom+j);
                }
            }
        }
    ...
    int readData() {
        ...
        for (int j=0; j<ngeom; j++) {
            ...
            if (kerma_p[j] && !kerma_p[j]->setState(*data_in)) {
                return 108+2*(ngeom+j);
            }
        }
        if (ncg > 0) {
    ...
    int addState(istream &data) {
        ...
        for (int j=0; j<ngeom; j++) {
            ...
            if (kerma_p[j]) {
                EGS_ScoringArray ktmp_p(kerma_p[j]->regions());
                if (!ktmp_p.setState(data)) {
                    return 108+2*(ngeom+j);
                }
                (*kerma_p[j]) += ktmp_p;
            }
    ...
    void outputKermaResults(EGS_Float F) {
        ...
        for (int j=0; j<ngeom; j++) {
            ...
            /****************************************************/
            /* Output total kerma in whole scoring volume */
            /****************************************************/
    ...
    void selectPhotonMFP(EGS_Float &dpmfp) {
        ...
        // the ray-tracing. Photons touching an exclusion region are
    ...
    int startNewShower() {
        ...
        if (current_case != last_case) {
            ...
            for (int j=0; j<ngeom; j++) {
                ...
                if (kerma_p[j]) {
                    kerma_p[j]->setHistory(current_case);
                }
            }
            if (flug) {
                for (int j=0; j<ngeom; j++) {
...
private:
    ...
    EGS_Float Eph_sc_p;  // Mean scoring photon energy
    EGS_Float Nsc;     // Number of scoring photons
    EGS_Float Nsc_p;   // Number of scoring primary photons
...
int EGS_KermaApplication::initScoring() {
    ...
    if (options) {
        ...
        while ((aux = options->takeInputItem("calculation geometry"))) {
            ...
            /* Read region-list inputs as strings, not vector<int>: entries
             * may be region LABELS (set via 'set label' in the geometry
             * definition -- e.g. EGS_CDGeometry auto-expanding a label for
             * every placement) as well as literal region numbers.
             * getNumberRegions()/getLabelRegions() below resolve both from
             * the same string -- see egs_base_geometry.{h,cpp}. This needs
             * the geometry object *before* region-list parsing, so the
             * geometry lookup that used to run after this switch has moved
             * up here. (Restores label support dropped by 90ddf75c "Add
             * primary kerma scoring to egs_kerma", which rewrote this
             * function against a base that predated it.) */
            string cavString;
            ...
            if (!g) {
                ...
                case incremental: {
                    ...
                    // Turn off sorting of the region list since it's not
                    // just regions -- entries are (start, end, delta).
            ...
            if (err || err1) {
                ...
                // g was already resolved above (region-list parsing needs it
                // for label lookups); err1 would be set and we wouldn't be
                // here if it were null, so no need to re-look it up.
                {
                    int nreg = g->regions();
    ...
    /***************************************************/
    /* Ausgab calls after photon interactions to mark  */
    /* scattered photons.                              */
    /*                                                 */
    /* NOTE: Primary particles defined as particles    */
    /*        that suffer any type of interaction.     */
    /***************************************************/
    int call;
...
void EGS_KermaApplication::describeSimulation() {
    ...
    for (int j=0; j<ngeom; j++) {
        ...
        if (active_med[j] >= 0)
           ...
           egsInformation("     Sensitive regions in vacuum :");
        outputSensitiveRegions(j,is_sensitive[j]);

        if (kerma_r[j]) {
            egsInformation("     Scoring kerma in %d regions\n",n_scoring_r[j]);
        }
        else {
